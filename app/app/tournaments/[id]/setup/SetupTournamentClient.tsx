    ...
    <>
      ...
      {nonMultipleOf4 && (
        <div className="rounded-xl border border-[#333333]/10 bg-[#333333]/5 p-3 text-sm text-[#333333]/70">
          Ce nombre de joueurs n&apos;est pas un multiple de 4 : certains seront
  ...
  if (!isDraft) {
    return (
      <div className="min-w-[360px] px-4 py-6">
        <div className="mx-auto max-w-lg space-y-6">{body}</div>
      </div>
    );
  }

  /* Brouillon : colonne flex + liste scrollable + CTA sous la liste (max-h = zone utile sous header, au-dessus de la tab bar) */
  return (
    <div className="flex max-h-[calc(100dvh-8.5rem)] min-h-0 w-full min-w-[360px] flex-col overflow-hidden">
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-6 [-webkit-overflow-scrolling:touch]">
        <div className="mx-auto max-w-lg space-y-6">{body}</div>
      </div>
      <div className="shrink-0 border-t border-[#333333]/10 bg-white/95 px-4 pt-3 shadow-[0_-4px_24px_rgba(0,0,0,0.06)] backdrop-blur-md pb-[max(0.75rem,env(safe-area-inset-bottom))]">
